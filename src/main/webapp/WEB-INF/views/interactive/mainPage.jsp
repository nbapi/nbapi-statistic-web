<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>

<!DOCTYPE html>
<html>
<head>
<title></title>
<link href="<c:url value="/resources/css/dialog/base.css"></c:url>"
	rel="stylesheet" type="text/css">
</head>
<body background="">
	<input type="hidden" id="baseUrl" value="${pageContext.request.contextPath}"/>
	<form class="form-horizontal ng-pristine ng-valid">
		<div class="main-box clearfix">

			<div class="main-box-body clearfix">
				<div class="span12">
					<div class="tabs-wrapper profile-tabs">
						<ul class="nav nav-tabs">

							<li id="li-tab-count" class="active">
								<a id="a-home" data-target="#tab-count" data-toggle="tab">自定义查询</a>
							</li>

						</ul>
						<div class="tab-content">

							<div class="tab-pane fade active in" id="tab-count">
								<div class="row row-input">
									<div class="btn-group">
										<label>已存在的表:</label><br>
										<c:forEach items="${tables}" var="t">
                                            ${t}	
                                        </c:forEach>
                                        <br><br>
									</div>
									<div id="txt" class="row">
									 	请输入SQL语句:<br>
										<textarea id="sql" rows="5" cols="80" placeholder="请输入单条查询SQL，不需要分号，请限制返回条数(limit)；数据仓库查询通常较慢，请耐心等待"></textarea>
										<button type="button" id="queryBtn">查询</button>
									</div>
									<div class="table-responsive">
										<table class="table-striped table-condensed table-bordered table-hover table-responsive">
											<caption>执行结果</caption>
											<thead id="theadtitle">
											</thead>
											<tbody id="tbodydata">
											</tbody>
										</table>
									</div>
								</div>
							</div>
							
						</div>
					</div>
				</div>
			</div>

		</div>
	</form>
	<script src="<c:url value="/resources/js/interactive/interactivePage.js"></c:url>"></script>
</body>
</html>